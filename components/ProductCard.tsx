      ...
      <Link href={`/product/${product.slug}`} className="block">
        <div className="relative aspect-[4/5] overflow-hidden rounded-2xl">
          <Image
            ...
            className="object-contain px-2 transition-transform duration-700 ease-out group-hover:scale-[1.04] sm:px-2"
          />
          {product.isNew && (
      ...
      <div className=" flex items-start justify-between gap-5">
        <div>
          <Link href={`/product/${product.slug}`}>
            <h3 className="font-serif text-base font-medium text-ink transition-colors group-hover:text-clay">
              {product.title}
            </h3>
          </Link>
          <p className="eyebrow mt-1 text-[10px]!">{product.tagline}</p>
        ...
        <Link
          ...
          className="mt-0.5 grid h-8 w-8 shrink-0 place-items-center rounded-full border border-ink/15 text-ink transition hover:border-olive hover:bg-[#383927] hover:text-cream-100"
        >
